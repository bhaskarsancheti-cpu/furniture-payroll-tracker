...
document.getElementById('addAdvanceForm').addEventListener('submit', (e) => {
    e.preventDefault();
...
document.getElementById('editAdvanceForm').addEventListener('submit', (e) => {
    e.preventDefault();
...
function markAdvanceSettled(advanceId) {
...
function initializeFirebase() {
    ...
    try {
        ...
        showToast('Firebase connection failed. Check credentials.', 'error');
        renderDashboard();
        setTimeout(() => {
            showFirebaseSetupModal();
        }, 1000);
...
document.getElementById('firebaseConfigForm').addEventListener('submit', (e) => {
    ...
    const config = {
        ...
        databaseURL: document.getElementById('firebaseDbUrl').value,
...
function loadDataFromFirebase() {
    ...
    const dbRef = firebaseDB.ref('furnitureWorkshop');
    dbRef.once('value', (snapshot) => {
        ...
        if (data) {
            ...
            renderDashboard();
            showToast('Data loaded successfully from Firebase', 'success');
        } else {
            syncDataToFirebase();
...
function syncDataToFirebase() {
    ...
    firebaseDB.ref('furnitureWorkshop').set({
        employees: employees,
