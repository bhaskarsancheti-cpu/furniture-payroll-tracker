/* app.js - Firebase-enabled drop-in for your original index.html
   IMPORTANT:
   1) Keep your original index.html intact (do not replace it).
   2) Replace only app.js with this file.
   3) Paste your Firebase config into the firebaseConfig object below.
   4) This version uses Firestore collections: employees, attendance, dailyLabor, advances.
*/
...
(function () {
  // ---- Minimal safe initialization check ----
  const hasFirebase = firebaseConfig && firebaseConfig.projectId;
  let db = null;

  // Local in-memory model fallback (will be overwritten by Firestore if enabled)
  let employees = [
    {
      id: 1,
      name: "Dharmendra",
  ...
  let currentUser = "You"; // kept for compatibility with your UI
  let firebaseConnected = false;

  // Utility functions (kept same as your original app)
  ...
  // Payroll calc (keeps original logic)
  ...
  // ------- Renderers (exposed globally so existing HTML onclick handlers keep working) -------
  ...
  window.renderAttendanceRecords = function (employee) {
    try {
      ...
      if (!records || records.length === 0) { container.innerHTML = '<p style="color: var(--color-text-secondary);">No attendance records for this month.</p>'; return; }
  ...
  // ------------------ Attendance modal logic (submit writes to firestore or local arrays) ------------------
  ...
  (function attachDailyLabHandler() {
    ...
    laborForm.addEventListener('submit', async (e) => {
      ...
      if (hasFirebase && firebaseConnected) {
        try { await db.collection('dailyLabor').doc(String(id)).set(entry); }
        catch (err) { console.error('dailyLabor write failed', err); showToast('Save failed', 'error'); return; }
      } else {
  ...
  window.renderAdvances = function () {
    try {
      ...
      const totalPending = pendingAdvances.reduce((sum, adv) => sum + ((adv.amount || 0) - (adv.amount_deducted || 0)), 0);
      const employeesWithAdvances = new Set(pendingAdvances.map(a => a.employee_id)).size;
      const oldestAdvance = pendingAdvances.length ? Math.floor((new Date() - new Date(pendingAdvances.reduce((oldest, a) => (new Date(a.date_given) < oldest ? new Date(a.date_given) : oldest), new Date()))) / (1000*60*60*24)) : '-';
      const settledThisMonth = advances.filter(a => a.status === 'Settled' && a.settled_date && a.settled_date.startsWith(selectedMonth)).reduce((s,a)=>s+(a.amount||0),0);

      document.getElementById('totalPendingAdvances') && (document.getElementById('totalPendingAdvances').textContent = formatCurrency(totalPending));
      document.getElementById('employeesWithAdvances') && (document.getElementById('employeesWithAdvances').textContent = employeesWithAdvances || 0);
      document.getElementById('oldestAdvance') && (document.getElementById('oldestAdvance').textContent = oldestAdvance === '-' ? '-' : oldestAdvance + ' days');
      document.getElementById('settledThisMonth') && (document.getElementById('settledThisMonth').textContent = formatCurrency(settledThisMonth));
      ...
      if (!tbody) return;
      if (filtered.length === 0) { tbody.innerHTML = '<tr><td colspan="9" style="text-align:center;color:var(--color-text-secondary)">No advances found.</td></tr>'; return; }
      tbody.innerHTML = filtered.map(adv => {
  ...
  (function attachAddAdvanceForm() {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      if (hasFirebase && firebaseConnected) {
        try { await db.collection('advances').doc(String(id)).set(entry); }
        catch (err) { console.error('advance write failed', err); showToast('Save failed', 'error'); return; }
      } else {
        advances.push(entry);
        renderAdvances();
  ...
  window.renderSummary = function () {
    try {
      ...
      if (monthLabor.length > 0) rows.push(`<tr style="background:var(--color-bg-2);"><td><strong>Daily Laborers</strong></td><td>Various</td><td>-</td><td>${Math.round(laborHours)} hrs</td><td>-</td><td>-</td><td>-</td><td><strong>${formatCurrency(laborCost)}</strong></td></tr>`);
      ...
      rows.push(`<tr style="background:var(--color-bg-3);font-weight:bold;"><td colspan="7" style="text-align:right;"><strong>TOTAL PAYROLL</strong></td><td><strong>${formatCurrency(totalPayroll)}</strong></td></tr>`);
  ...
  // ------------------ Firestore connect and listeners (if firebaseConfig provided) ------------------
  async function connectFirestore() {
    if (!hasFirebase) {
      console.warn('No firebaseConfig provided — running in local-only mode.');
      // initial renders for local-only
    ...
    try {
      ...
      db.collection('employees').onSnapshot(snapshot => {
        ...
        try { renderDashboard(); renderEmployeeCards(); renderEmployeeSelect(); renderSummary(); } catch(e){}
      });
      ...
      // initial read fallback completed
      console.log('Connected to Firestore (client). Realtime listeners attached.');

    } catch (err) {
      console.error('Failed to connect to Firestore:', err);
      showToast('Firestore connect failed — running local-only', 'error');
      firebaseConnected = false;
      try { renderDashboard(); renderEmployeeCards(); renderEmployeeSelect(); renderDailyLabor(); renderSummary(); renderAdvances(); } catch (e) {}
    }
  }

  // small helper to write docs in Firestore
  async function setDocSafe(collectionName, id, data) {
    if (!hasFirebase || !firebaseConnected) {
      console.warn('setDocSafe local fallback', collectionName, id);
      // update local arrays for immediate UX
      ...
      // re-render local UI
      try { renderDashboard(); renderEmployeeCards(); renderEmployeeDetail(currentEmployee); renderDailyLabor(); renderSummary(); renderAdvances(); } catch(e){}
      return;
    }
    try {
  ...
  // ------------------ delete/edit modals (helpers) ------------------
  window.openEditModal = function (attendanceId) {
    // Find attendance and populate modal for edit
  ...
  window.openEditAdvanceModal = function (advId) {
    ...
    // Simple inline prompt edits (keeps UI minimal)
  ...
  // Status menu toggle (attendance status change)
  window.toggleStatusMenu = function (ev, attendanceId) {
    ev.stopPropagation();
    const menu = ['Present','Half-day','Leave','Absent'];
    const record = attendanceLog.find(a => Number(a.id) === Number(attendanceId));
    if (!record) return;
  ...
  // Attach small tab behavior (index.html already has it but ensure idempotence)
  (function attachTabs() {
    try {
      ...
      tabs.forEach(tab => tab.addEventListener('click', () => {
        ...
        document.getElementById(tabName) && document.getElementById(tabName).classList.add('active');
        // call respective render
        if (tabName === 'dashboard') renderDashboard();
        if (tabName === 'employees') renderEmployeeSelect();
        if (tabName === 'employee-management') {/* not implemented separate */ }
        if (tabName === 'daily-labor') renderDailyLabor();
        if (tabName === 'advances') renderAdvances();
        if (tabName === 'summary') renderSummary();
        if (tabName === 'export') {/* nothing */ }
      }));
    } catch (e) { /* ignore */ }
  })();

  // Initial renders (local data will be replaced by firestore listeners if connected)
  try { renderDashboard(); renderEmployeeCards(); renderEmployeeSelect(); renderDailyLabor(); renderSummary(); renderAdvances(); } catch (e) {}

  // Connect to firebase if config present
  if (hasFirebase) {
    // Lazy load firebase if necessary (compat included in index.html expected)
    try {
      if (!window.firebase) throw new Error('Firebase SDK not loaded in index.html. Add firebase-app and firebase-firestore compat scripts.');
      connectFirestore();
    } catch (err) {
      console.error(err);
    ...
    console.log('No Firebase config: running local-only. Data will not persist on refresh.');
  }

  // expose some items for debugging
  window._APP = {
